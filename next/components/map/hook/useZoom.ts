...
export default function  useZoom(svgRef: RefObject<SVGSVGElement | null>, ratio: Ratio, setRatio: (r: Ratio) => void, base: Point, setBase: (p: Point) => void, zoomFactor:number = DEFAULT_ZOOM_FACTOR) {
  ...
  const onWheel = (event: React.WheelEvent<SVGSVGElement>) => {
    ...
    zoom(midPoint, direction, Math.sqrt(Math.sqrt(zoomFactor)));
