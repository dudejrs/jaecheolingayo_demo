    ...
    return (
      <div className="flex flex-wrap content-between content justify-end"> 
        <div className="flex flex-wrap gap-2 h-150 w-75 overflow-scroll" >
          {
            tags.map((tag, i) => 
              (<label key={i} className={`relative  text-sm inline-flex items-center cursor-pointer mb-0 px-2 py-1 border rounded-md  ${selectedTags.includes(tag.id) ? "bg-blue-500 text-white": "bg-[var(--gray-50)]/20 border-[var(--gray-50)]/50"}`}>
                  <input type="checkbox" className={`absolute w-0 h-0 opacity-0"`} key={i} value={tag.name} checked={selectedTags.includes(tag.id)} onChange={() => handleCheckboxChange(tag.id)} />
                  {`#${tag.name}`}
              </label>))
          }
        </div>
